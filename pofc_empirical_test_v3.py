...
Design summary (all outputs are regenerated by this script, RNG_SEED = 2025)
- Synthetic portfolio, N = 420. POFC adoption follows a logistic propensity
  model on budget, duration, historical CPI and ERP legacy score. The
  data-generating propensity score is stored and used both as a covariate
  and to build inverse-probability weights (IPW).
- Irreducible noise sigma_eps = 0.065; overrun measurement noise 0.020;
  DT measurement error sd = 2 days; external shocks hit 7% of projects.
- All four models (Ridge, Logistic Regression, Random Forest, Gradient
  Boosting) are trained with IPW sample weights. Hold-out and 5-fold CV
  metrics are IPW-weighted, i.e. evaluated on the balanced pseudo-population.
- Covariate balance (SMD) is reported before and after IPW weighting.
- Welch t, Mann-Whitney U, Cohen's d; bootstrap 95% CIs (B = 5,000 for the
  DT difference, B = 3,000 for KPI deltas).
- Monte Carlo EAC simulation (N = 50,000); DSO mu + 2 sigma alert rule.
"""
...
# IPW-weighted balance (post-weighting SMD)
_t   = (df.forecast_model == "POFC").values
_w   = np.where(_t, 1/df.propensity_score, 1/(1-df.propensity_score))
def _wm(x, w): return np.sum(w*x)/np.sum(w)
def _wv(x, w): m = _wm(x, w); return np.sum(w*(x-m)**2)/np.sum(w)
smd_ipw = []
for col in balance_cols:
    x = df[col].values
    a, b, wa, wb = x[~_t], x[_t], _w[~_t], _w[_t]
    smd_ipw.append(round(float(abs(_wm(a,wa)-_wm(b,wb)) /
                         np.sqrt((_wv(a,wa)+_wv(b,wb))/2)), 3))
df_balance["SMD_IPW"] = smd_ipw
df_balance["Balanced_IPW"] = ["yes" if v < 0.20 else "no" for v in smd_ipw]
print("\n  Post-IPW balance (SMD_IPW):")
print(df_balance[["Feature","SMD","SMD_IPW"]].to_string(index=False))
print(f"  Features with SMD_IPW < 0.20: {(df_balance.SMD_IPW < 0.20).sum()}/10;"
      f"  < 0.10: {(df_balance.SMD_IPW < 0.10).sum()}/10")

# ERP legacy score vs DT within each regime
for _r in ["Manual", "POFC"]:
    _s = df[df.forecast_model == _r]
    _rr, _pp = stats.pearsonr(_s.erp_legacy_score, _s.decision_latency_days)
    print(f"  r(ERP legacy, DT | {_r}) = {_rr:.3f}  (p = {_pp:.3f})")
...
print(f"\n  Top-12 Feature Importances (GBR):")
...
kpi_defs = [
    ...
    ("Report Prep Time (days)",     ("report_days_manual","report_days_pofc"), np.mean, "days"),
...
for label, col, fn, unit in kpi_defs:
    cm_, cp_ = col if isinstance(col, tuple) else (col, col)
    mv = df.loc[df.forecast_model=="Manual", cm_].values
    pv = df.loc[df.forecast_model=="POFC",   cp_].values
...
for label, vm, vp in [
    ...
    d = (vp-vm)/abs(vm)*100
    fmt = ",.4f" if vm < 1 else ",.1f"
    print(f"  {label:<32} {vm:>12{fmt}} {vp:>12{fmt}} {d:>+9.1f}%")
...
ax.set_title(f"Figure 2 — DT vs Overrun (colour = propensity score)\n"
             f"Pearson r = {corr_dt:.4f}  (synthetic portfolio, N = {N})",
             fontsize=11)
...
for ax, name, col in zip(axes,
    ...
    ax.set_title(f"{name}  IPW-weighted R²={r2_score(y_te,pr,sample_weight=w_te):.3f}", fontsize=11)
...
fi_top = fi.head(12)
fig, ax = plt.subplots(figsize=(10,6))
cols_fi = [C4 if ("propensity" in f or "legacy" in f)
           else (C1 if (f.startswith("dt_") or "decision_latency" in f or "erp" in f)
...
ax.set_title("Figure 5 — Feature Importances\n"
             "Navy = DT/ERP   Teal = EVM   Gold = propensity score / ERP legacy   Purple = other",
             fontsize=11)
...
axes[0].set_title(f"Confusion Matrix — {best_name} (unweighted counts)")
...
for nm, col in [("Logistic Regression",C3),("Random Forest",C4),
    ...
    a_ = roc_auc_score(y_te_c,pr,sample_weight=w_te_c)
    axes[1].plot(f_,t_,color=col,lw=1.8,label=f"{nm}  AUC={a_:.3f}")
axes[1].plot([0,1],[0,1],"k--",lw=1)
axes[1].set_xlabel("FPR"); axes[1].set_ylabel("TPR")
axes[1].set_title("ROC Curves — All Models (IPW-weighted AUC)"); axes[1].legend(fontsize=9)
plt.tight_layout()
...
mv_=[df.loc[df.forecast_model=="Manual","decision_latency_days"].mean(),
     ...
     df.loc[df.forecast_model=="Manual","opex_actual_kUSD"].mean()]
...
ax.set_title("Figure 8 — POFC vs Manual: KPI Comparison (synthetic portfolio)\n"
             "(bootstrap 95% CIs reported in the console output, Section 6)",fontsize=11)
...
=============================================
The synthetic dataset (N=420) replicates a non-random adoption mechanism:
POFC assignment follows a logistic propensity model on budget, duration,
historical CPI and ERP legacy score. The resulting propensity score is
included as a covariate in all model specifications, and inverse-
probability weights (IPW) are applied when training all four models and
when computing hold-out and cross-validated performance metrics. Before
weighting, seven of ten covariates have SMD < 0.20 (budget, duration and
ERP legacy score are imbalanced); after IPW weighting all ten covariates
have SMD < 0.20 (see SMD_IPW in pofc_balance_table.csv). External shocks
affecting 7% of projects and an irreducible noise floor (sigma_eps =
0.065) leave POFC projects with a residual overrun rate of 16.5%. All
results are conditional on this simulated data-generating process and are
